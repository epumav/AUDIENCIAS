...
app.use(cors({
  origin(origin, callback) {
    // Permite solicitudes sin Origin (curl/health checks) y el frontend configurado.
...
function accountConfig(index) {
  return {
    index,
    email: required(`ACCOUNT${index + 1}_EMAIL`),
    refreshToken: required(`ACCOUNT${index + 1}_REFRESH_TOKEN`)
  };
}
...
app.get("/api/week", async (req, res) => {
  try {
    ...
    // Los límites se envían en UTC. El frontend solicita semanas lunes-domingo.
